...
def call_deepseek(market_summary: str, fundamentals_text: str, approved_view: dict | None) -> tuple[str | None, str]:
    ...
    prompt = (
        ...
        f"- 从CSV中筛选view_season=2026/27的记录\n"
        f"- 主观点: 发布日期最新的一条2026/27年度机构预估\n"
        f"- 补充观点(可选): 同属2026/27年度且方向明显不同的另一条\n"
        f"- 不得使用2025/26年度数据(如ISO过剩224.4万吨)来判断2026/27\n"
        f"- 不得跨榨季年度沿用\n"
        f"- 如果CSV中有view_season=2026/27的记录，禁止写'全球供需暂无新增数据'\n"
        f"- 推荐写法: '全球供需方面，Czarnikow最新预计2026/27年度全球糖市将出现约140万吨的小幅过剩'\n"
        f"- 如果只有方向无数值: 'StoneX最新预计2026/27年度全球糖市将从过剩转向短缺'\n\n"
        f"【巴西分析规则——核心是因果归因】\n"
