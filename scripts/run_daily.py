...
"""
白糖日报生成系统 - 主程序
用法: py scripts/run_daily.py [--date YYYY-MM-DD]
...
def daily_change_phrase(change_pct: float) -> str:
    """Return the report-safe daily change phrase generated by Python."""
    if change_pct > 0:
...
def _insert_after_domestic_start(text: str, sentence: str) -> str:
    sentence = sentence.rstrip("。") + "。"
    if sentence in text:
        return text
    if "国内方面，" in text:
        return text.replace("国内方面，", f"国内方面，{sentence}", 1)
    return text.rstrip() + "\n\n" + f"国内方面，{sentence}"
...
def build_structured_fundamentals(cache_records: list[dict], researcher_text: str | None,
    ...
    结构: 国际供需 → 巴西 → 印度 → 泰国 → 国内25/26 → 26/27面积 → 广西天气 → 进口/政策 → 国内价格判断
    """
    ...
    for section, title in [
        ...
        if section == "next_season_26_27_area":
            ...
            lines.append(
                f"  [researcher_confirmed_view][confirmed] 中国/{title}: {fact} (文本) "
                f"| 来源:研究员确认 | editable_by_model:false | allow_model_override:false"
            )
        if section == "next_season_26_27_weather":
            fact = researcher_fact("guangxi_weather_current_impact", "广西当前天气暂未对甘蔗生长和产量预期形成实质影响")
            lines.append(
                f"  [researcher_confirmed_view][confirmed] 中国/广西天气影响: {fact} (文本) "
                f"| 来源:研究员确认 | editable_by_model:false | allow_model_override:false"
            )
    ...
    lines.append("  请基于以上国际和国内全部信息，形成自然段结论。")
    lines.append("  不生成具体开仓点、止损位、目标位。")

    return "\n".join(lines)


def _infer_supply_direction(view: dict) -> str:
    direction = view.get("view_direction", "")
    if direction:
        return direction
    text = " ".join(str(view.get(k, "")) for k in ["value_or_fact", "text_value", "notes", "indicator"])
    if "过剩" in text:
        return "surplus"
    if "短缺" in text or "转为短缺" in text:
        return "deficit"
    indicator = str(view.get("indicator", "")).lower()
    # Standard CSV rows currently keep values but not custom view_direction columns.
    if "czarnikow" in indicator:
        return "surplus"
    if "iso" in indicator or "stonex" in indicator:
        return "deficit"
    return ""


def _format_supply_view(view: dict, main: bool = False) -> str:
    inst = view.get("institution") or str(view.get("indicator", "")).replace("全球供需-", "") or "机构"
    direction = _infer_supply_direction(view)
    value = view.get("surplus_deficit_value") or str(view.get("value", "")).replace("万吨", "")
    if direction == "surplus":
        body = f"将出现约{value}万吨的小幅过剩" if value else "将出现小幅过剩"
    elif direction == "deficit":
        body = f"短缺约{value}万吨" if value else "将转为短缺"
    elif direction == "balanced":
        body = "供需基本平衡"
    else:
        body = str(view.get("value_or_fact") or view.get("text_value") or "有新预估")
    prefix = f"{inst}最新预计" if main else f"{inst}预计"
    return f"{prefix}2026/27年度全球糖市{body}"


def build_global_supply_sentence(cache_records: list[dict]) -> str:
    """Build the global supply sentence from verified 2026/27 institution views."""
    candidates = []
    for r in cache_records:
        if r.get("country") != "宏观":
            continue
        if not str(r.get("indicator", "")).startswith("全球供需-"):
            continue
        season = r.get("view_season") or r.get("season")
        if season != "2026/27":
            continue
        candidates.append(r)
    if not candidates:
    ...
    candidates.sort(key=lambda r: (r.get("published_at") or r.get("data_date") or ""), reverse=True)
    main = candidates[0]
    parts = [_format_supply_view(main, main=True)]

    main_direction = _infer_supply_direction(main)
    supplemental = next((r for r in candidates[1:] if _infer_supply_direction(r) != main_direction), None)
    if supplemental:
        parts.append(_format_supply_view(supplemental, main=False))
        return "国际方面，全球供需方面，" + "；".join(parts) + "，机构观点存在分歧。"
    return "国际方面，全球供需方面，" + parts[0] + "。"


def enforce_global_supply_sentence(text: str, cache_records: list[dict]) -> str:
    sentence = build_global_supply_sentence(cache_records)
    if not sentence:
        return text
    pattern = r"国际方面，全球供需方面，[^。]*。"
    if re.search(pattern, text):
        return re.sub(pattern, sentence, text, count=1)
    if text.startswith("国际方面，"):
        return text.replace("国际方面，", sentence, 1)
    return sentence + text


def fix_repeated_section_leads(text: str) -> str:
    """Fix repeated lead words only on summary sentences, preserving paragraph starts."""
    if not text:
        return ""
    text = re.sub(r"(?<!^)(?<!\n)国际方面，全球糖市预计", "目前，全球糖市预计", text)
    text = re.sub(r"(?<!^)(?<!\n)国际方面，国际糖价预计", "目前，国际糖价预计", text)
    text = re.sub(r"(?<!^)(?<!\n)国内方面，国内", "当前来看，国内", text)
    text = re.sub(r"(?<!^)(?<!\n)国内方面，郑糖", "当前来看，郑糖", text)
    return text
...
def _enforce_researcher_fixed_rules(text: str) -> str:
    """Apply researcher-confirmed facts that the model is not allowed to override."""
    if not text:
        return ""

    area_fact = researcher_fact("china_26_27_planting_area", "26/27榨季甘蔗种植面积确定增加")
    weather_fact = researcher_fact(
        "guangxi_weather_current_impact",
        "广西当前天气暂未对甘蔗生长和产量预期形成实质影响",
    )

    # Remove label-style judgment leads while preserving the actual sentence content.
    text = re.sub(r"(?:SR2609|郑糖|内盘)判断[：:为，,]?\s*", "", text)

    # Replace forbidden area "no data" wording with the fixed researcher view.
    text = re.sub(
        r"26/27榨季(?:国内|广西、云南|广西和云南|广西|云南)?甘蔗?种植面积(?:方面)?"
        r"(?:暂无数据|暂无新的已验证信息|变化不明确|暂无明确数据)[。；;]?",
        area_fact + "。",
        text,
    )
    text = re.sub(
        r"26/27榨季种植面积(?:方面)?(?:暂无数据|暂无新的已验证信息|变化不明确|暂无明确数据)[。；;]?",
        area_fact + "。",
        text,
    )

    # Keep Guangxi weather wording restrained and prevent local events becoming a province-wide crop impact.
    weather_patterns = [
        r"广西[^。；;]{0,80}(?:强降雨|降雨|积水|苗黄|苗弱|根系受损)[^。；;]{0,80}"
        r"(?:对[^。；;]{0,20}(?:甘蔗生长|产量预期|产量|单产)[^。；;]{0,20}(?:形成|构成|带来)[^。；;]{0,20}(?:影响|压力|不利影响))[。；;]?",
        r"广西[^。；;]{0,60}(?:天气已经形成实质减产|全区苗情明显恶化|天气对产量形成明显压力)[。；;]?",
    ]
    for pat in weather_patterns:
        text = re.sub(pat, weather_fact + "。", text)

    if area_fact not in text:
        text = _insert_after_domestic_start(text, area_fact)
    if weather_fact not in text:
        text = _insert_after_domestic_start(text, f"天气方面，{weather_fact}")
...
def _check_logic_consistency(text: str, fundamentals_input: str) -> list[str]:
    ...
    # ── 0. 研究员固定规则检查 ──
    if re.search(r"(?<!日)(?<!涨跌)(涨幅|跌幅)\d+(?:\.\d+)?%", text):
        issues.append("日涨跌幅格式错误：涨幅/跌幅前缺少'日'")

    area_no_data_patterns = [
        "26/27榨季种植面积暂无数据",
        "26/27榨季甘蔗种植面积暂无数据",
        "种植面积暂无新的已验证信息",
        "种植面积变化不明确",
    ]
    for term in area_no_data_patterns:
        if term in text:
            issues.append("研究员已确认26/27榨季种植面积增加，不得输出暂无数据")

    if "SR2609判断" in text or "郑糖判断" in text or "内盘判断" in text:
        issues.append("基本面正文不得出现'SR2609判断''郑糖判断''内盘判断'标签")
    ...
    if re.search(r"国际方面，全球供需方面", international_segment) and re.search(r"国际方面，全球糖市预计", international_segment):
        issues.append("国际总结句重复使用'国际方面'，应改为'目前，全球糖市预计……'")
    if international_segment.count("国际方面") > 1:
        issues.append("同一国际自然段内重复使用'国际方面'作为句首")

    domestic_segment = text.split("国内方面", 1)[1] if "国内方面" in text else ""
    if domestic_segment.count("国内方面") > 0:
        issues.append("同一国内自然段内重复使用'国内方面'作为句首")

    if "印度不新增出口" in text or "当前无新增出口" in text or "无新增出口" in text:
        india_bad = ["形成压制", "供应压力", "利空"]
        india_match_all = re.search(r"印度.{0,180}?(?=泰国|国际方面|国内方面|$)", text, re.S)
        india_scope = india_match_all.group(0) if india_match_all else text
        if any(term in india_scope for term in india_bad):
            issues.append("印度不新增出口属于利多或支撑，不得写成压制、供应压力或利空")

    guangxi_weather_forbidden = [
        "广西天气已经形成实质减产",
        "全区苗情明显恶化",
        "天气对产量形成明显压力",
        "广西天气对产量形成明显压力",
    ...
    for term in guangxi_weather_forbidden:
        if term in text:
            issues.append("广西当前天气尚未形成研究员确认的实质影响，不得扩大为全区减产风险")

    # ── 1. 印度出口政策方向检查 ──
    has_no_new_export = any(kw in fundamentals_input for kw in [
        "暂无新增出口", "出口政策无新增信号", "暂无新增配额",
        "政府尚未宣布扩大出口", "出口政策暂无", "不新增出口",
        "无新增出口", "当前无新增出口",
    ])
...
def call_deepseek(market_summary: str, fundamentals_text: str, approved_view: dict | None) -> tuple[str | None, str]:
    ...
    prompt = (
        ...
        f"第二段(国内): 25/26现实供给→26/27种植面积→广西天气→进口→政策→国内价格判断\n"
        f"基本面最后必须分别写国际结论和国内结论，不得混成一句。\n\n"
        f"【研究员确认固定事实——模型不得覆盖】\n"
        f"1. 26/27榨季甘蔗种植面积确定增加，不得改写为暂无数据。\n"
        f"2. 广西当前天气暂未对甘蔗生长和产量预期形成实质影响，不得扩大为全区减产风险。\n"
        f"3. 印度不新增出口属于利多或支撑，不属于供应压制。\n"
        f"4. 国际判断与国内判断必须分开写。\n"
        f"5. 删除'SR2609判断''郑糖判断''内盘判断'等标签式文字。\n"
        f"6. 不得修改Python生成的市场涨跌幅措辞。\n"
        f"7. 前文已经使用'国际方面'时，国际总结句必须使用'目前'开头，不得再次使用'国际方面'。\n"
        f"8. 不得在同一段中重复以'国际方面'或'国内方面'作为句首。\n"
        f"固定事实JSON: {json.dumps(config.get('researcher_confirmed_facts', {}), ensure_ascii=False)}\n\n"
        f"【表达风格——参照研究员样例】\n"
        ...
        f"- 国内段落顺序: 25/26现实供给→26/27种植面积→广西天气→进口→政策→国内价格判断\n"
        ...
        f"- 26/27种植面积固定写: '26/27榨季甘蔗种植面积确定增加。'\n"
        f"- 广西天气固定写克制表述: '广西当前天气暂未对甘蔗生长和产量预期形成实质影响。'\n"
        f"- 不使用基差自动推导现货支撑\n"
        f"- 不生成具体开仓点、止损位、目标位\n\n"
        f"【结论拆分规则】\n"
        f"- 国际段结尾单独写: '目前，全球糖市预计……，但……，国际糖价预计……。'\n"
        f"- 前文已经使用'国际方面'时，国际总结句必须使用'目前'开头，不得再次使用'国际方面'\n"
        f"- 不得在同一段中重复以'国际方面'作为句首\n"
        f"- 国内段结尾自然衔接，不得在同一段中重复以'国内方面'作为句首\n"
        f"- 禁止用一个'综合来看'把国际、国内和{TARGET_CONTRACT}压缩到同一句\n\n"
        f"【风险提示】\n"
        f"- 固定写: '宏观、政策、天气、进口量。'\n"
        f"- 不得自动生成具体事件\n\n"
        f"【格式】\n"
    ...
    # ── 清理响应 ──
    text = _enforce_researcher_fixed_rules(_clean_model_response(raw_text))
    if not text:
...
def _is_logic_consistency_issue(issue: str) -> bool:
    markers = [
        "日涨跌幅", "研究员已确认", "SR2609判断", "郑糖判断", "内盘判断",
        "国际方面", "国内方面", "印度不新增出口", "广西当前天气", "广西天气",
    ]
    return any(m in issue for m in markers)
...
def generate_report(target_date: str, market: dict, fundamentals_ai: str | None,
    ...
    if all_ok:
        status = "success"
    elif any(_is_logic_consistency_issue(issue) for issue in (consistency_issues or [])):
        status = "failed_logic_consistency"
    else:
...
def run(target_date: str | None = None) -> Path | None:
    ...
    elif any(_is_logic_consistency_issue(issue) for issue in (issues or [])):
        final_status = "failed_logic_consistency"
    else:
        final_status = "needs_contract_confirmation"
    logger.info("=" * 60)
    logger.info("日报生成完成")
    ...
    # ── 更新前端 JSON ──
    try:
        if os.getenv("SUGAR_DAILY_SKIP_AUTO_PUBLISH") == "1":
            logger.info("已跳过前端JSON更新和自动推送 (SUGAR_DAILY_SKIP_AUTO_PUBLISH=1)")
            return out_path
